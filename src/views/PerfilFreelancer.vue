  ...
  if (cadastrado) {
    return {
      ...
      services: cadastrado.precoServico
        ? [{
            title: cadastrado.profissao,
            priceRange: formatarPreco(cadastrado.precoServico),
            duration: 'A combinar'
          }]
        : [],
      reviews: []
...
function handleRequestQuote() {
  ...
  const freelancerSelecionado = {
    id: profissional.value.id || '',
    nome: profissional.value.name || '',
    profissao: profissional.value.title || '',
    cidade: profissional.value.location || '',
    descricao: profissional.value.bio || '',
    anosExperiencia: profissional.value.experiences?.[0]?.period || '',
    categorias: profissional.value.skills || [],
    fotoPerfil: profissional.value.avatar || null,
    email: profissional.value.email || ''
  }

  localStorage.setItem(
    'freelancerSelecionado',
    JSON.stringify(freelancerSelecionado)
  )
...
<template>
  <div class="page-container">
    ...
    >
      ...
      <div
        v-if="profissionaisContratacoesExibidos.length"
        class="contratacoes-grid"
      >
        <ProfessionalCard
      ...
      <p
        v-else
        class="contratacoes-vazio"
      >
        {{
          abaContratacoes === 'espera'
            ? 'Nenhum profissional em espera.'
            : 'Nenhum freelancer contratado.'
        }}
      </p>
    </section>
  </div>
